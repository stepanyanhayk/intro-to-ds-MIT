...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.tiles[(tile_x, tile_y)] <= capacity:
            self.tiles[(tile_x, tile_y)] = 0
        else:
            self.tiles[(tile_x, tile_y)] -= capacity
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        if self.tiles[(math.floor(m), math.floor(n))] == 0:
            return True
        else:
            return False
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        if self.is_position_in_room(pos) and self.is_position_furnished(pos) is False:
            return True
        else:
            return False
    ...
    def get_num_tiles(self):
        ...
        tiles_count = 0
        for tile in self.tiles.keys():
            if self.is_tile_furnished(tile[0], tile[1]) is False:
                tiles_count += 1
        return tiles_count
    ...
    def get_random_position(self):
        ...
        while True:
            ...
            if self.is_position_valid(pos) is True:
                return pos
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        new_position = self.get_robot_position().get_new_position(self.get_robot_direction(), self.speed)
...
#       of a 20x20 room?
# Their performance is approximately the same with a litter advantage of StandardRobot.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions
#       10x30, 20x15, 25x12, and 50x6?
# It takes significantly less time for Standard Robot to clean the 80% of the room.
#
